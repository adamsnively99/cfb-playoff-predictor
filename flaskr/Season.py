...
class Season:
    ...
    def simulate_season(self):
        for game in self.schedule:
            ...
            self.simulate_game(home_team, away_team, is_neutral_site, self.rating_stdev, self.avg_top_team, game)
            self.teams.update({home_team_name: home_team})
            self.teams.update({away_team_name: away_team})
        self.simulate_conference_champs(self.rating_stdev, self.avg_top_team)

    def simulate_game(self, home_team, away_team, is_neutral_site, rating_stdev, avg_top_team, game):
        ...
        if home_team.name == game.simulate_winner(home_team_odds):  # Home team wins in simulation
            self.update_after_game(home_team, away_team, top_team_odds_home, top_team_odds_away)
            return home_team
        else:  # Home Team loses
            self.update_after_game(away_team, home_team, top_team_odds_away, top_team_odds_home)
            return away_team

    def update_after_game(self, winning_team, losing_team, top_team_odds_winner, top_team_odds_loser):
    ...
    def simulate_conference_champs(self, rating_stdev, avg_top_team):
        for conference in Constants.CONFERENCES:
            division_a = conference
            division_b = Constants.CONFERENCES.get(division_a)
            team_a_name = self.get_best_conference_record(division_a, '')
            team_b_name = self.get_best_conference_record(division_b, team_a_name)
            team_a = self.teams.get(team_a_name)
            team_b = self.teams.get(team_b_name)
            champion = self.simulate_game(team_a, team_b, True, rating_stdev, avg_top_team,
    ...
    def get_best_conference_record(self, division, exclusion_team):
        wins = -1
